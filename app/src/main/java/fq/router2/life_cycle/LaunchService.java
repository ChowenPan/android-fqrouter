...
public class LaunchService extends IntentService {
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        sendBroadcast(new LaunchingIntent(getStr(R.string.status_check_root), 5));
        boolean rooted = ShellUtils.checkRooted();
        LogUtils.i("rooted: " + rooted);
        sendBroadcast(new LaunchingIntent(getStr(R.string.status_check_existing_process), 10));
        ...
        if ("VPN".equals(runningAs)) {
            sendBroadcast(new LaunchingIntent(getStr(R.string.status_restart_vpn), 0));
            LogUtils.i("Restart manager");
            try {
                ManagerProcess.kill();
                Thread.sleep(1000);
                if (ManagerProcess.exists()) {
                    handleFatalError(getStr(R.string.status_failed_to_restart_vpn));
        ...
        if (rooted) {
            sendBroadcast(new LaunchingIntent(getStr(R.string.status_about_to_launch_in_root_mode), 20));
        } else {
            if (Build.VERSION.SDK_INT < 14) {
                handleFatalError(getStr(R.string.status_root_is_requried_below_4_0));
                return;
            }
            sendBroadcast(new LaunchingIntent(getStr(R.string.status_about_to_launch_in_vpn_mode), 20));
            File managerLogFile = new File("/data/data/fq.router2/log/fqsocks.log");
            if (managerLogFile.exists() && !managerLogFile.canWrite()) {
                handleFatalError(LogUtils.e(getStr(R.string.status_root_permission_lost)));
                return;
            }
        }
        try {
    ...
    private void deployAndLaunch() {
        sendBroadcast(new LaunchingIntent(getStr(R.string.status_clean_environment), 25));
        try {
        ...
        sendBroadcast(new LaunchingIntent(getStr(R.string.status_starting_manager), 45));
        ...
        try {
            ...
            for (int i = 0; i < 30; i++) {
                ...
                if (hasProcessExited(process)) {
                    return getStr(R.string.status_failed_to_launch);
                }
                sendBroadcast(new LaunchingIntent(getStr(R.string.status_starting_manager), 45 + i));
                sleepOneSecond();
            }
            if (apnName != null && WAP_APN_LIST.contains(apnName.trim().toLowerCase())) {
                return getStr(R.string.status_3g_apn_has_proxy);
            }
            return getStr(R.string.status_timed_out);
        } catch (Exception e) {
