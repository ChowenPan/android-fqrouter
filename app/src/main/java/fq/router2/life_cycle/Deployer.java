...
public class Deployer {
    ...
    public String deploy() {
        ...
        if (foundPayloadUpdate) {
            context.sendBroadcast(new LaunchingIntent(getStr(R.string.status_clear_old_files), 30));
            try {
                try {
                    ManagerProcess.kill();
                } catch (Exception e) {
        ...
        context.sendBroadcast(new LaunchingIntent(getStr(R.string.status_copy_files), 35));
        try {
            copyBusybox();
            makeExecutable(ShellUtils.BUSYBOX_FILE);
            copyPayloadZip();
        ...
        context.sendBroadcast(new LaunchingIntent(getStr(R.string.status_unzip_files), 40));
        try {
            unzipPayloadZip();
        } catch (Exception e) {
            ...
            try {
                if (ShellUtils.execute("getprop", "ro.product.cpu.abi").trim().equals("x86")) {
                    return getStr(R.string.x86_is_not_supported);
                }
            } catch (Exception e2) {
                // ignore
            }
